...
// Modified distributeData function for timestep-major ordering
double* distributeData(int rank, const SubDomain* subdomain, double* globalData,
    ...
    if (rank == 0) {
            ...
            for (int z = subdomain->tempStartZ; z <= subdomain->tempEndZ; z++) {
                for (int y = subdomain->tempStartY; y <= subdomain->tempEndY; y++) {
                    for (int x = subdomain->tempStartX; x <= subdomain->tempEndX; x++) {
                        ...
                        // For timestep-major: (t * totalDomainSize + globalIdx)
                        localData[t * localPointCount + localIdx] = globalData[t * totalDomainSize + globalIdx];
                        localIdx++;
                    }
                }
            }
        ...
        for (int p = 1; p < pX * pY * pZ; p++) {
            ...
            int recvPointCount = recvSubdomain.tempWidth * recvSubdomain.tempHeight * recvSubdomain.tempDepth;
            int sendDataSize = recvPointCount * timeSteps;
                ...
                for (int z = recvSubdomain.tempStartZ; z <= recvSubdomain.tempEndZ; z++) {
                    for (int y = recvSubdomain.tempStartY; y <= recvSubdomain.tempEndY; y++) {
                        for (int x = recvSubdomain.tempStartX; x <= recvSubdomain.tempEndX; x++) {
                            int globalIdx = getLinearIndex(x, y, z, nX, nY, nZ);
                            // For timestep-major: (t * totalDomainSize + globalIdx)
                            tempBuffer[t * recvPointCount + bufIdx] = globalData[t * totalDomainSize + globalIdx];
                            bufIdx++;
                        }
                    }
                }
