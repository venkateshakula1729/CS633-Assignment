...
// Optimized file reading function (supports both text and binary files)
// Optimized file reading function (supports text and binary files)
...
bool isBinaryFile(const char* filename) {
    ...
    float val;
    int count = fread(&val, sizeof(float), 1, fp);
    fclose(fp);
    return count == 1;
}


float* readLocalDataParallelIO(const char* binFile, const SubDomain* subdomain,
    int nX, int nY, int nZ, int timeSteps, MPI_Comm comm) {
MPI_File fh;
MPI_Datatype fileType;
int localPoints = subdomain->tempWidth * subdomain->tempHeight * subdomain->tempDepth;
int totalLocalSize = localPoints * timeSteps;

float* localData = (float*)malloc(totalLocalSize * sizeof(float));
if (!localData) {
printf("Memory allocation failed\n");
return NULL;
}

// Global sizes for subarray: Z × Y × X × timeSteps
int sizes[4]    = {nZ, nY, nX, timeSteps};
...
// Create 4D subarray datatype
MPI_Type_create_subarray(4, sizes, subsizes, starts, MPI_ORDER_C, MPI_FLOAT, &fileType);
MPI_Type_commit(&fileType);

MPI_File_open(comm, binFile, MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
MPI_File_set_view(fh, 0, MPI_FLOAT, fileType, "native", MPI_INFO_NULL);
MPI_File_read_all(fh, localData, totalLocalSize, MPI_FLOAT, MPI_STATUS_IGNORE);
MPI_File_close(&fh);
...
int main(int argc, char** argv) {
    ...
    localData = readLocalDataParallelIO(inputFile, &subdomain, nX, nY, nZ, timeSteps, MPI_COMM_WORLD);
    if (!localData) {
